 ...
 * Build: http://modernizr.com/download/#-fontface-backgroundsize-generatedcontent-csstransforms-csstransforms3d-csstransitions-touch-cssclasses-teststyles-testprop-testallprops-prefixes-domprefixes
 */
window.Modernizr = function(a, b, c) {
    ...
    return A(""), i = k = null, e._version = d, e._prefixes = n, e._domPrefixes = q, e._cssomPrefixes = p, e.testProp = function(a) {
      return E([a])
  ...
  function(a, b) {
    ...
    "getBoundingClientRect" in c.documentElement ? vc = function(a, b, c, d) {
      ...
      }), a.jQuery = a.$ = f, typeof define == "function" && define.amd && define
      .amd.jQuery && define("jquery", [], function() {
        return f
      })
  }(window),
  function(a, b) {
  ...
  function(a) {
    ...
    function j(b, c, d) {
      var f = {};
      f.url = e(c.getResponseHeader("X-PJAX-URL") || d.requestUrl);
      var g = a(b);
    ...
    var c = a.pjax = function(b) {
      ...
      var g = f(b.url).hash,
        ...
        m = b.success,
        n = b.error,
        o = b.context = h(b.container);
      b.data || (b.data = {}), b.data._pjax = o.selector;
      var q;
      b.beforeSend = function(a, c) {
        c.timeout > 0 && (q = setTimeout(function() {
          p("pjax:timeout", [a, b]) && a.abort("timeout")
        }, c.timeout), c.timeout = 0), a.setRequestHeader("X-PJAX", "true"), a.setRequestHeader("X-PJAX-Container", o.selector);
        var d;
        if (i) {
          d = i.apply(this, arguments);
          if (d === !1) return !1
        }
        if (!p("pjax:beforeSend", [a, c])) return !1;
        b.requestUrl = f(c.url).href
      ...
      var r = c.xhr;
      r && r.readyState < 4 && (r.onreadystatechange = a.noop, r.abort()), c.options = b;
      var r = c.xhr = a.ajax(b);
      return r.readyState > 0 && (a(document).trigger("pjax", [r, b]), b.push && !b.replace && (l.push(c.state.id, o.clone(!0, !0).contents()), window.history.pushState(null, "", b.url)), p("pjax:start", [r, b]), p("start.pjax", [r, b]), p("pjax:send", [r, b])), c.xhr
    };
    ...
    b.infinitescroll = function(c, d, e) {
      ...
      _binding: function(b) {
        var d = this,
          e = d.options;
        e.v = "2.0b2.111027";
        if (!!e.behavior && this["_binding_" + e.behavior] !== c) {
          this["_binding_" + e.behavior].call(this);
          return
        }
        if (b !== "bind" && b !== "unbind") return this._debug("Binding value  " + b + " not valid"), !1;
        b == "unbind" ? this.options.binder.unbind("smartscroll.infscr." + d.options.infid) : this.options.binder[b]("smartscroll.infscr." + d.options.infid, function() {
          d.scroll()
      ...
      _create: function(d, e) {
        ...
        this.options = f;
        var g = b(f.nextSelector).attr("href");
        return g ? (f.path = this._determinepath(g), f.contentSelector = f.contentSelector || this.element, f.loading.selector = f.loading.selector || f.contentSelector, f.loading.msg = b('<div id="infscr-loading"><img alt="Loading..." src="' + f.loading.img + '" /><div>' + f.loading.msgText + "</div></div>"), (new Image).src = f.loading.img, f.pixelsFromNavToBottom = b(document).height() - b(f.navSelector).offset().top, f.loading.start = f.loading.start || function() {
          b(f.navSelector).hide(), f.loading.msg.appendTo(f.loading.selector).show(f.loading.speed, function() {
            beginAjax(f)
          })
        }, f.loading.finished = f.loading.finished || function() {
          f.loading.msg.fadeOut("normal")
        }, f.callback = function(a, d) {
          !!f.behavior && a["_callback_" + f.behavior] !== c && a["_callback_" + f.behavior].call(b(f.contentSelector)[0], d), e && e.call(b(f.contentSelector)[0], d, f)
      ...
      _determinepath: function(b) {
        var d = this.options;
        if (!d.behavior || this["_determinepath_" + d.behavior] === c) {
          if (!d.pathParse) {
            if (b.match(/^(.*?)\b2\b(.*?$)/)) b = b.match(/^(.*?)\b2\b(.*?$)/).slice(1);
            else if (b.match(/^(.*?)2(.*?$)/)) {
              if (b.match(/^(.*?page=)2(\/.*|$)/)) return b = b.match(/^(.*?page=)2(\/.*|$)/).slice(1), b;
              b = b.match(/^(.*?)2(.*?$)/).slice(1)
            } else {
              if (b.match(/^(.*?page=)1(\/.*|$)/)) return b = b.match(/^(.*?page=)1(\/.*|$)/).slice(1), b;
          ...
          return this._debug("pathParse manual"), d.pathParse(b, this.options.state.currPage + 1)
        }
        this["_determinepath_" + d.behavior].call(this, b);
        return
      },
      _error: function(b) {
        var d = this.options;
        if (!!d.behavior && this["_error_" + d.behavior] !== c) {
          this["_error_" + d.behavior].call(this, b);
          return
        }
        b !== "destroy" && b !== "end" && (b = "unknown"), this._debug("Error", b), b == "end" && this._showdonemsg(), d.state.isDone = !0, d.state.currPage = 1, d.state.isPaused = !1, this._binding("unbind")
      },
      _loadcallback: function(e, f) {
        var g = this.options,
        ...
        if (!!g.behavior && this["_loadcallback_" + g.behavior] !== c) {
          this["_loadcallback_" + g.behavior].call(this, e, f);
          return
        }
        switch (i) {
          case "done":
          ...
          case "no-append":
            g.dataType == "html" && (f = "<div>" + f + "</div>", f = b(f).find(g.itemSelector));
            break;
          case "append":
            var k = e.children();
            if (k.length == 0) return this._error("end");
            j = document.createDocumentFragment();
            while (e[0].firstChild) j.appendChild(e[0].firstChild);
            this._debug("contentSelector", b(g.contentSelector)[0]), b(g.contentSelector)[0].appendChild(j), f = k.get()
        }
        g.loading.finished.call(b(g.contentSelector)[0], g);
        if (g.animate) {
          var l = b(a).scrollTop() + b("#infscr-loading").height() + g.extraScrollPx + "px";
          b("html,body").animate({
            scrollTop: l
          }, 800, function() {
        ...
        g.animate || (g.state.isDuringAjax = !1), h(this, f)
      },
      _nearbottom: function() {
        var e = this.options,
          f = 0 + b(document).height() - e.binder.scrollTop() - b(a).height();
        return !e.behavior || this["_nearbottom_" + e.behavior] === c ? (this._debug("math:", f, e.pixelsFromNavToBottom), f - e.bufferPx < e.pixelsFromNavToBottom) : this["_nearbottom_" + e.behavior].call(this)
      },
      _pausing: function(b) {
        var d = this.options;
        if (!d.behavior || this["_pausing_" + d.behavior] === c) {
          b !== "pause" && b !== "resume" && b !== null && this._debug("Invalid argument. Toggling pause value instead"), b = !b || b != "pause" && b != "resume" ? "toggle" : b;
          switch (b) {
            case "pause":
              d.state.isPaused = !0;
              break;
        ...
        this["_pausing_" + d.behavior].call(this, b);
        return
      },
      _setup: function() {
        var b = this.options;
        if (!b.behavior || this["_setup_" + b.behavior] === c) return this._binding("bind"), !1;
        this["_setup_" + b.behavior].call(this);
        return
      },
      _showdonemsg: function() {
        var d = this.options;
        if (!!d.behavior && this["_showdonemsg_" + d.behavior] !== c) {
        ...
        }), d.errorCallback.call(b(d.contentSelector)[0], "done")
      },
      _validate: function(c) {
        for (var d in c)
          if (d.indexOf && d.indexOf("Selector") > -1 && b(c[d]).length === 0) return this._debug("Your " + d + " found no elements."), !1;
        return !0
      },
      bind: function() {
        this._binding("bind")
      ...
      retrieve: function(d) {
        var e = this,
          ...
          g = f.path,
          h, i, j, k, l, d = d || null,
          m = d ? d : f.state.currPage;
        beginAjax = function(d) {
          d.state.currPage++, e._debug("heading into ajax", g), h = b(d.contentSelector).is("table") ? b("<tbody/>") : b("<div/>"), j = g.join(d.state.currPage), k = d.dataType == "html" || d.dataType == "json" ? d.dataType : "html+callback", d.appendCallback && d.dataType == "html" && (k += "+callback");
          switch (k) {
            case "html+callback":
              e._debug("Using HTML via .load() method"), h.load(j + " " + d.itemSelector, null, function(b) {
                e._loadcallback(h, b)
              });
              break;
            case "html":
              e._debug("Using " + k.toUpperCase() + " via $.ajax() method"), b.ajax({
                url: j,
                dataType: d.dataType,
                complete: function(b, c) {
                  l = typeof b.isResolved != "undefined" ? b.isResolved() : c === "success" || c === "notmodified", l ? e._loadcallback(h, b.responseText) : e._error("end")
                }
              });
              break;
            case "json":
              e._debug("Using " + k.toUpperCase() + " via $.ajax() method"), b.ajax({
                ...
                success: function(a, b, f) {
                  l = typeof f.isResolved != "undefined" ? f.isResolved() : b === "success" || b === "notmodified";
                  if (d.appendCallback)
                    if (d.template != c) {
                      var g = d.template(a);
                      h.append(g), l ? e._loadcallback(h, g) : e._error("end")
                    } else e._debug("template must be defined."), e._error("end");
                  else l ? e._loadcallback(h, a) : e._error("end")
                },
        ...
        if (!!f.behavior && this["retrieve_" + f.behavior] !== c) {
          this["retrieve_" + f.behavior].call(this, d);
          return
        }
        if (f.state.isDestroyed) return this._debug("Instance is destroyed"), !1;
        f.state.isDuringAjax = !0, f.loading.start.call(b(f.contentSelector)[0], f)
      },
      scroll: function() {
        var b = this.options,
          d = b.state;
        if (!!b.behavior && this["scroll_" + b.behavior] !== c) {
          this["scroll_" + b.behavior].call(this);
          return
        }
        if (d.isDuringAjax || d.isInvalidPage || d.isDone || d.isDestroyed || d.isPaused) return;
      ...
      switch (e) {
        case "string":
          ...
          this.each(function() {
            ...
            if (!a) return !1;
            if (!b.isFunction(a[c]) || c.charAt(0) === "_") return !1;
            a[c].apply(a, f)
          });
          break;
        case "object":
  ...
  function() {
    window.paper || (window.paper = {}), paper.setup = function() {
      ...
      if (paper.SCROLL_INFINITELY) return paper.$posts.infinitescroll({
        ...
        var b;
        return b = $(a).css("opacity", 0), b.imagesLoaded(function() {
          return b.animate({
            opacity: 1
          }), paper.books.build(), paper.$posts.masonry("reload")
        })
      })
    }, $(function() {
      return paper.setup()
  ...
  function() {
    window.paper || (window.paper = {}), paper.features = {
      load: function() {
        var a = this;
        return $.ajax({
          url: "/tagged/featured",
          success: function(b) {
            ...
            while (l < a.posts.length && l < 6) {
              ...
              if (p === "bio") j = g.html();
              else if (p === "photo") d = g.find("img.post-image"), d.each(function() {
                ...
                if (g.find("iframe").length > 0) j = g.find(".post-content");
                else {
                  j = $("<a />").attr({
                    href: g.attr("data-permalink"),
                    "class": "audio_link"
                  }).append('<span class="audio_player_icon">&nbsp;</span>'), c = g.find(".audio-player");
                  if (c.attr("data-artist") || c.attr("data-track") || c.attr("data-album")) m = $("<ul />"), c.attr("data-artist") && m.append("<li>" + decodeURI(c.attr("data-artist")) + "</li>"), c.attr("data-track") && m.append("<li>" + decodeURI(c.attr("data-track")) + "</li>"), c.attr("data-album") && m.append("<li>" + decodeURI(c.attr("data-album")) + "</li>"), m.appendTo(j);
                  c.attr("data-art") && $("<img />").attr("src", c.attr("data-art")).appendTo(j)
                }
              else p === "answer" ? (n = $("<a />").attr("href", g.attr("data-permalink")), n.append(g.find(".post-content")), j = $('<div class="post-pad" />').append(n)) : j = $('<div class="post-pad" />').append(g.find(".post-title")).append(g.find(".post-content"));
            ...
            while (l--) h = $("<em />").html("•").attr("data-index", a.list.find(".item").length - l - 1), h.on("click", function(a) {
              return $("#features .carousel").carousel(parseInt($(this).attr("data-index")))
            }), f.append(h);
            return f.find("em:first-child").addClass("on"), $("#features .carousel").on("slid", function(a) {
  ...
  function() {
    window.paper || (window.paper = {}), paper.books = {
      build: function() {
        if (typeof Modernizr != "undefined" && Modernizr !== null && Modernizr.csstransforms) return $(".photoset_wrap").length === 0 && paper.$posts.masonry(), $(".photoset_wrap").each(function(a) {
          var b, c, d, e, f, g;
          ...
          if (d.data("notebook") != null) return c = $(this).closest(".post").addClass("notebooked")
        })
      }
    }, paper.Notebook = function() {
      function a(a, b) {
        this.container = a, $(this.container).hide(), this.pages = [], this.currentPage = 0, this.permalink = this.container.getAttribute("data-permalink"), this.settings = b || {}, this.settings.useRotation !== !1 && (this.settings.useRotation = !0), this.settings.xMovement !== !1 && (this.settings.xMovement = !0), this.settings.xMovement !== !1 && (this.settings.yMovement = !0), this.setMaximumHeight(), this.extractSourcesFromContainer(), this.writeMarkup(), this.appendElement(), $(this.container).show()
      }
      return a.prototype.setMaximumHeight = function() {
        ...
        var a, b, c;
        b = this.container.querySelectorAll("img"), this.sources = [], a = b.length, c = [];
        while (a--) c.push(this.sources.push(b[a].src));
        return c
      }, a.prototype.setImageHeights = function(a, b, c, d, e, f) {
        return $("<img>").attr("src", a).load(function() {
          var a, g, h, i;
          g = Math.round($(b).width() * this.height / this.width), h = $(b).width(), a = (i = $(c).parents(".feature_content").length > 0) != null ? i : {
            "true": !1
          }, g > f && (g = f, h = Math.round(this.width * g / this.height)), $(b).css({
            height: g,
            width: h
          }).attr("data-height", g).attr("data-width", h), g > parseInt($(c).attr("data-height")) && $(c).attr("data-height", g), h > parseInt($(c).attr("data-width")) && $(c).attr("data-width", h), d && ($(c).css("height", $(c).attr("data-height")), $(c).width() > parseInt($(c).attr("data-width")) && $(c).attr("data-width", $(c).width()), a && $(c).css("width", $(c).attr("data-width")), $(c).find("div").each(function() {
            var a, b;
            return b = Math.round(($(c).attr("data-height") - $(this).attr("data-height")) / 2), a = Math.round(($(c).attr("data-width") - $(this).attr("data-width")) / 2), $(this).css({
              marginTop: b,
              marginLeft: a
            })
          }));
          if (d && e) return paper.$posts.masonry()
        })
      }, a.prototype.writeMarkup = function() {
        var a, b, c, d, e;
        this.element = document.createElement("div"), this.element.className = "notebook", this.element.id = "notebook_" + parseInt(Math.random() * 1e5), this.element.setAttribute("data-height", 0), this.element.setAttribute("data-width", 0), a = this.sources.length, e = [];
        while (a--) c = document.createElement("div"), c.style.backgroundImage = "url(" + this.sources[a] + ")", b = (d = a === 0) != null ? d : {
          "true": !1
        }, this.setImageHeights(this.sources[a], c, this.element, b, this.settings.lastNotebook, this.max_height), c.className = "notebook-page page-style-" + parseInt(Math.random() * 5), c.setAttribute("data-page", this.sources.length - a), c.style.zIndex = a + 1e3, this.pages.push(c), this.element.appendChild(c), c.addEventListener("touchstart", this, !1), c.addEventListener("touchmove", this, !1), c.addEventListener("touchend", this, !1), c.addEventListener("click", this, !1), Modernizr.touch ? e.push(void 0) : e.push(this.dragify(c));
        return e
        ...
        var b;
        a.preventDefault(), this.target = a.target, b = this.target.style, b.webkitTransitionDuration = b.MozTransitionDuration = b.msTransitionDuration = b.OTransitionDuration = b.transitionDuration = "0s", this.start = {
          pageX: a.touches[0].pageX,
          pageY: a.touches[0].pageY,
          time: Number(new Date)
        }, this.deltaX = 0, this.deltaY = 0, this.originalTransform = $(this.target).css("-webkit-transform");
        if (this.settings.parent != null) return this.settings.parent.addClass("dragging")
      }, a.prototype.onTouchMove = function(a) {
        var b;
        return a.touches.length > 1 || a.scale && a.scale !== 1 ? !0 : (a.preventDefault(), a.stopPropagation(), this.settings.xMovement && (this.deltaX = a.touches[0].pageX - this.start.pageX), this.settings.yMovement && (this.deltaY = a.touches[0].pageY - this.start.pageY), b = this.target.style, b.webkitTransform = b.MozTransform = b.msTransform = b.OTransform = b.transform = this.originalTransform + (" translate(" + this.deltaX + "px, " + this.deltaY + "px)"))
      }, a.prototype.onTouchEnd = function(a) {
        var b, c, d = this;
        this.distance = Math.sqrt(this.deltaX * this.deltaX + this.deltaY * this.deltaY), this.deltaT = Number(new Date) - this.start.time, this.rect = this.element.getBoundingClientRect(), this.width = this.rect.right - this.rect.left, this.height = this.rect.bottom - this.rect.top, c = this.target.style, c.webkitTransitionProperty = c.MozTransitionProperty =
          c.msTransitionProperty = c.OTransitionProperty = c.transitionProperty = "all", c.webkitTransitionDuration = c.MozTransitionDuration = c.msTransitionDuration = c.OTransitionDuration = c.transitionDuration = ".4s", c.webkitTransform = c.MozTransform = c.msTransform = c.OTransform = c.transform = "rotate(0deg)", this.target.style.top = this.target.style.left = "", window.setTimeout(function() {
            if (d.settings.parent != null) return d.settings.parent.removeClass("dragging")
          }, 400), b = Math.abs(this.deltaY) > this.height / 2 || Math.abs(this.deltaX) > this.width / 2 || this.distance > 20 && this.deltaT < 250;
        if (b) return this.flip();
        if (this.deltaT < 500 && this.distance < 2 && !$("body").hasClass("show")) return window.location.href = this.permalink
      }, a.prototype.onDragStart = function(a) {
        var b;
        this.dragged = !0, this.target = a.target, b = this.target.style, b.webkitTransitionDuration = b.MozTransitionDuration = b.msTransitionDuration = b.OTransitionDuration = b.transitionDuration = "0s", this.start = {
          pageX: a.pageX,
          pageY: a.pageY,
          time: Number(new Date)
        ...
        return this.settings.parent != null && this.settings.parent.addClass("dragging"), this.deltaX = a.pageX - this.start.pageX, this.deltaY = a.pageY - this.start.pageY
      }, a.prototype.onDragEnd = function(a) {
        return this.onTouchEnd(a)
      }, a.prototype.onClick = function(a) {
        if (this.dragged) return this.dragged = !1, !0;
        if (!$("body").hasClass("show")) return window.location.href = this.permalink
      }, a.prototype.flip = function() {
        var a, b, c, d, e, f, g, h, i = this;
        return d = this.deltaY / this.distance, b = this.deltaX / this.distance, c = this.distance, a = (this.width > this.height ? this.width : this.height) * 1.2, f = Math.floor(a * b), g = Math.floor(a * d), h = .2, e = this.target.style, e.webkitTransitionDuration = e.MozTransitionDuration = e.msTransitionDuration = e.OTransitionDuration = e.transitionDuration = h + "s", Modernizr.touch ? e.webkitTransform = e.MozTransform = e.msTransform = e.OTransform = e.transform = "translate(" + f + "px," + g + "px)" : (e.left = f + "px", e.top = g + "px"), window.setTimeout(function() {
          return i.afterFlip()
        }, h * 1e3)
      }, a.prototype.afterFlip = function() {
        var a, b, c, d, e;
        a = this.pages.length, this.currentPage += 1, this.currentPage === a && (this.currentPage = 0), e = [];
        while (a--) c = this.pages[a], b = parseInt(c.style.zIndex) + 1, d = c.style, c.style.zIndex = b < this.pages.length + 1e3 ? b : 1e3, e.push(d.top = d.left = d.webkitTransform = d.MozTransform = d.msTransform = d.OTransform = d.transform = "");
        return e
      }, a
    }()
  }.call(this),
  function() {
    window.paper || (window.paper = {}), paper.MovablePage = function() {
      function a(a, b) {
        switch (typeof a) {
          case "string":
        ...
        b || (b = {}), this.nextSelector = b.nextSelector || "a.pagination-older", this.prevSelector = b.prevSelector || "a.pagination-newer", this.fragmentSelector = b.fragmentSelector || "#posts", this.activateArrows(), this.element.addEventListener("touchstart", this, !1), this.element.addEventListener("touchmove", this, !1), this.element.addEventListener("touchend", this, !1)
      }
      return a.prototype.next = function() {
        var a;
        return a = this.element.style, a.webkitTransitionDuration = a.MozTransitionDuration = a.msTransitionDuration = a.OTransitionDuration = a.transitionDuration = this.getVelocityAdjustedTransitionDuration() / 1e3 + "s", a.webkitTransitionProperty = a.MozTransitionProperty = a.msTransitionProperty = a.OTransitionProperty = a.transitionProperty = "all", a.webkitTransform = a.MozTransform = a.msTransform = a.OTransform = a.transform = "translate3d(-100%,0,0)", a.opacity = "0", this.load(this.nextLink.href)
      }, a.prototype.prev = function() {
        var a;
        return a = this.element.style, a.webkitTransitionDuration = a.MozTransitionDuration = a.msTransitionDuration = a.OTransitionDuration = a.transitionDuration = this.getVelocityAdjustedTransitionDuration() / 1e3 + "s", a.webkitTransitionProperty = a.MozTransitionProperty = a.msTransitionProperty = a.OTransitionProperty = a.transitionProperty = "all", a.webkitTransform = a.MozTransform = a.msTransform = a.OTransform = a.transform = "translate3d(100%,0,0)", a.opacity = "0", this.load(this.prevLink.href)
      }, a.prototype.getVelocityAdjustedTransitionDuration = function() {
        var a, b, c;
        return this.start == null ? 300 : (this.deltaT = Number(new Date) - this.start.time, c = Math.abs(this.deltaX / this.deltaT), a = $(window).width() - Math.abs(this.deltaX), b = Math.abs(this.deltaT * a / this.deltaX), b > 500 && (b = 500), b)
        ...
        return window.setTimeout(function() {
          return $ && $.support.pjax ? $.pjax({
            ...
            success: function(a) {
              var c, d, e;
              return e = b.element.style, e.webkitTransitionProperty = e.MozTransitionProperty = e.msTransitionProperty = e.OTransitionProperty = e.transitionProperty = "opacity", e.opacity = "1", e.webkitTransform = e.MozTransform = e.msTransform = e.OTransform = e.transform = "", d = $(a).find(".pagination"), c = $(a).find("ol.notes"), $(".note-wrap").html(c), $("#pagination-posts").replaceWith(d), b.activateArrows(), paper.setup()
            }
        ...
        if (this.prevLink != null) return this.prevLink.onclick = function(b) {
        ...
        var b;
        if (a.touches.length > 1 || a.scale && a.scale !== 1) return !0;
        this.deltaX = a.touches[0].pageX - this.start.pageX, typeof this.isScrolling == "undefined" && (this.isScrolling = !!(this.isScrolling || Math.abs(this.deltaX) < Math.abs(a.touches[0].pageY - this.start.pageY)));
        if (!this.isScrolling) return a.preventDefault(), b = this.element.style, b.webkitTransform = b.MozTransform = b.msTransform = b.OTransform = b.transform = "translate3d(" + this.deltaX + "px, 0, 0)"
      }, a.prototype.onTouchEnd = function(a) {
        var b;
        return this.isScrolling ? !0 : (this.deltaX > 200 || this.deltaX > 20 && Number(new Date) - this.start.time < 250) && this.prevLink != null ? this.prev() : (this.deltaX < -200 || this.deltaX < -20 && Number(new Date) - this.start.time < 250) && this.nextLink != null ? this.next() : (b = this.element.style, b.webkitTransitionDuration = b.MozTransitionDuration = b.msTransitionDuration = b.OTransitionDuration = b.transitionDuration = ".5s", b.webkitTransform = b.MozTransform = b.msTransform = b.OTransform = b.transform = "")
      }, a
    }()
  }.call(this);
